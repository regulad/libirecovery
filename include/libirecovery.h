...
/* As irecv_usb_control_transfer(), but also reports how many bytes the host
   controller believes moved, via `transferred` (may be NULL), even when the
   transfer stalled or timed out -- neither backend's ordinary wrapper keeps
   that count. Return value is identical to irecv_usb_control_transfer()'s. */
